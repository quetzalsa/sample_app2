...
class UserTest < ActiveSupport::TestCase
  ...
  def setup
    @user = User.new(name: "example", email: "example@algo.com", password: "foobar", password_confirmation: "foobar")
  end
  ...
  test "email validation should accept valid adresses" do
    valid_adresses = %w[user@example.com USER@feo.COM A_US-ER@far.bar.con first.last@foo.jp aice+bob@buz.com]
  ...
  test "email validation should reject invalid adresses" do
    invalid_adresses = %w[foo@bar..com user@example,com user_at_foo.org user.name@example. foo@bar_baz.com foo@bar+baz.com]
